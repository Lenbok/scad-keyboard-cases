...
module reset_microswitch() {
    translate([0, 0, wall_thickness]) {
        cube([14, 6, 6], center = false);
        translate([4, 0, 1])  cube([2, 10, 4], center = false);
    }
}
...
module mini_usb_hole() {
    translate([0, 0, wall_thickness + 0.05]) {
        color("green") translate([0, -10, pcb_thickness/2]) cube([26, 20, pcb_thickness], center = true);
...
module micro_usb_hole() {
    translate([0, 0, wall_thickness + 0.01]) {
        color("green") translate([0, -7, pcb_thickness/2]) cube([14, 14, pcb_thickness], center = true);
        color("silver") {
